...
int app_on_http_req(app_t *self, libwebsocket *socket, char *request) {
	...
	else if (strcmp(request, "/js264-vnc.js") == 0) {
		libwebsockets_serve_http_file(self->server->context, socket, "client/js264-vnc.js", "text/javascript; charset=utf-8", NULL);
		return true;
	}
	else if (strcmp(request, "/h264bsd_canvas.js") == 0) {
		libwebsockets_serve_http_file(self->server->context, socket, "js/h264bsd_canvas.js", "text/javascript; charset=utf-8", NULL);
		return true;
	} 
	else if (strcmp(request, "/h264bsd_decoder.js") == 0) {
		libwebsockets_serve_http_file(self->server->context, socket, "js/h264bsd_decoder.js", "text/javascript; charset=utf-8", NULL);
		return true;
	}
	else if (strcmp(request, "/h264bsd_worker.js") == 0) {
		libwebsockets_serve_http_file(self->server->context, socket, "js/h264bsd_worker.js", "text/javascript; charset=utf-8", NULL);
		return true;
	}
	else if (strcmp(request, "/h264bsd_asm.js") == 0) {
		libwebsockets_serve_http_file(self->server->context, socket, "js/h264bsd_asm.js", "text/javascript; charset=utf-8", NULL);
		return true;
	}
	return false;
}
		...
		if( delta > wait_time ) {
			...
			double encode_time = timer_measure(encode_time) {
				...
				if( encoded_size ) {
					...
					server_broadcast(self->server, frame->data, encoded_size, server_type_binary);
